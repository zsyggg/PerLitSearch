...
6. Strictly limit the output length to between approximately 100 and {max_length} characters, absolutely not exceeding {max_length} characters.
...
class AsyncGeminiPortraitGenerator:
    def __init__(self, config=None, max_concurrent: int = 15):
        ...
        logger.info(f"异步生成器初始化: 模型={self.model_name}, 最大并发数={max_concurrent}")
    
    async def generate_portrait_with_retry(self, query_data: Dict, memory_features: List[str], 
                                         cognitive_data: Optional[Dict] = None, max_retries: int = 3) -> Dict:
        """带重试机制的画像生成"""
        ...
        for attempt in range(max_retries):
            try:
                portrait = await self.generate_portrait_async(query_text, memory_features)
                if attempt > 0:
                ...
                if attempt < max_retries - 1:
                    ...
                    logger.warning(f"查询 {query_id} 第 {attempt + 1} 次尝试失败，{wait_time}秒后重试: {e}")
    ...
    async def generate_portrait_async(self, query: str, memory_features: List[str]) -> str:
        """异步生成画像"""
        async with self.semaphore:
            ...
            if memory_features and len(memory_features) > 0:
                ...
                formatted_features = "No specific memory features available for this query."
            ...
            candidates = []
            temperatures = [0.3, 0.5] if self.generation_attempts >= 2 else [self.temperature]
    ...
    def _clean_and_truncate(self, text: str) -> str:
        ...
        text = text.strip()
        prefixes_to_remove = [
            "Okay, ", "Sure, ", "Certainly, ", "Here's the generated", "Here is the generated",
            "Generated Core Statement:", "Core Statement:", "Enhanced Query Representation:",
            "Output:", "The core statement is:", "This is the core statement:"
        ]
    ...
    def _evaluate_quality(self, text: str, query: str) -> float:
        ...
        content_score = 0.0
        core_verbs = ['define', 'explain', 'compare', 'analyze', 'list', 'show', 'find', 'identify']
        if any(verb in text.lower() for verb in core_verbs):
            content_score += 0.3
            
        technical_indicators = ['method', 'approach', 'algorithm', 'model', 'system', 'framework', 'technique']
...
def load_cognitive_features(cognitive_features_path: str) -> Dict[str, Dict]:
            ...
            try:
                ...
                if query_id:
                    ...
                    if line_num <= 5:  # 只打印前5条
                        logger.debug(f"加载认知特征 - query_id: {query_id} (type: {type(query_id)}), "
    ...
    class FlexibleDict(dict):
        ...
        def __contains__(self, key):
            return str(key) in self._str_dict or (isinstance(key, int) and key in self._int_dict)
...
async def process_file_async(input_path: str, output_path: str, cognitive_features_path: str, 
                            config, max_concurrent: int = 15, batch_size: int = 50):
    """异步处理文件"""
    ...
    with open(input_path, 'r', encoding='utf-8') as f:
        for line_num, line in enumerate(f, 1):
            try:
                ...
                if line_num <= 5:
                    query_id = data.get('query_id', '')
                    logger.debug(f"加载查询 - query_id: {query_id} (type: {type(query_id)})")
    ...
    for batch_idx in range(total_batches):
        ...
        for query_data in batch_queries:
            ...
            if query_id in cognitive_features:
                ...
                logger.debug(f"找到查询 {query_id} 的认知特征: {len(memory_features)} 条")
            else:
                queries_without_features += 1
                logger.debug(f"未找到查询 {query_id} 的认知特征")
            
            # 创建任务
            task = generator.generate_portrait_with_retry(query_data, memory_features, cognitive_data)
            tasks.append(task)
...
async def main():
    ...
    for dataset_name in datasets:
        ...
        cognitive_features_path = os.path.join(args.cognitive_dir, dataset_name, "cognitive_features_detailed.jsonl")
        ...
        try:
            await process_file_async(
                input_path, output_path, cognitive_features_path, 
                config, args.concurrent, args.batch
            )
        except Exception as e:
