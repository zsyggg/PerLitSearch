...
# Map local model identifiers to HuggingFace repositories for tokenization
MODEL_TOKENIZER_MAP = {
...
def resolve_tokenizer_name(model_name: str) -> str:
    """Return a valid repo name for AutoTokenizer loading."""
    mapped = MODEL_TOKENIZER_MAP.get(model_name.lower())
    if mapped:
        return mapped
...
class PersonalizedGenerator:
    def __init__(self, config=None):
        ...
        tokenizer_name = resolve_tokenizer_name(self.model_name)
        try:
            ...
            logger.warning(
                f"Could not load tokenizer for {tokenizer_name}: {e}. Falling back to GPT2 tokenizer"
            )
