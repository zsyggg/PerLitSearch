...
try:
    ...
    class DummyConfig:
        ...
        local_model_temperature=0.4; local_model_top_p=0.95; local_model_top_k=20;
        enable_thinking=False; local_model_max_tokens=350;
...
class PersonalizedGenerator:
    def __init__(self, config=None):
        ...
        if self.api_type == 'siliconflow':
            ...
            self.model_name = getattr(self.config, 'siliconflow_model', "deepseek-ai/DeepSeek-R1")
            logger.info(f"PersonalizedGenerator initialized with SiliconFlow API (Model: {self.model_name})")
        else:
            # Ollama API配置
            self.api_url = getattr(self.config, 'llm_base_url', "http://172.18.147.77:11434")
            self.model_name = getattr(self.config, 'llm_model', "llama3:8b")
            logger.info(f"PersonalizedGenerator initialized with Ollama API (Model: {self.model_name})")

        self.max_length = getattr(self.config, 'personalized_text_max_length', 300)
        logger.info(f"Generator using max text length constraint: {self.max_length} characters")

        self.local_model_max_tokens = getattr(self.config, 'local_model_max_tokens', 350)
        ...
        logger.info(f"Generator params: temp={self.temperature}, top_p={self.top_p}, top_k={self.top_k}, max_new_tokens={self.local_model_max_tokens}")
        ...
        if self.api_type == 'siliconflow':
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.warning(f"SiliconFlow API responded with status: {response.status_code}")
    ...
    def _llm_request(self, prompt: str, max_tokens: int = None, temperature: float = None, seed: int = None) -> str:
        """发送请求到API"""
        ...
        for attempt in range(retry_count + 1):
            try:
                ...
                if not result:
                    logger.warning(f"LLM returned an empty response for prompt starting with: {prompt[:100]}...")
                    return ""
                return result
                    
            except requests.exceptions.RequestException as e:
                logger.error(f"LLM request failed (attempt {attempt + 1}/{retry_count + 1}): {e}")
    ...
    def _build_unified_turn1_style_prompt(self, query: str, formatted_features: str) -> str:
        ...
        # 强化的长度限制提示
        prompt_lines = [
            f"CRITICAL REQUIREMENT: Output MUST be between {length_range} characters. This is MANDATORY.",
            "",
        ...
        if formatted_features and formatted_features != "No relevant memory features available.":
            prompt_lines.extend([
        ...
        prompt_lines.extend([
            "STRICT INSTRUCTIONS:",
            f"1. LENGTH CONSTRAINT: Your output MUST be {length_emphasis}, between {length_range} characters.",
            ...
            "",
            f"REMINDER: Output length MUST be {length_range} characters. Count carefully.",
            "",
    ...
    def generate_personalized_text(self, query: str, memory_results: Dict,
                                  previous_profile: Optional[str] = None,
                                  turn_id: Optional[int] = None) -> str:
        """生成个性化描述文本（单次生成，无评分）"""
        try:
            tagged_features_list = memory_results.get("tagged_memory_features", []) if isinstance(memory_results, dict) else []
            formatted_features = DynamicPromptTemplates.format_memory_features(tagged_features_list or [])

            prompt = self._build_unified_turn1_style_prompt(query, formatted_features)

            # 单次生成
            logger.info(f"Generating description for query '{query[:70]}...' (Max length: {self.max_length})")

            raw_response = self._llm_request(prompt, temperature=self.temperature, seed=self.llm_seed if self.use_fixed_seed else None)
            ...
            if not cleaned_text:
                logger.warning(f"Cleaned text was empty for query '{query[:70]}...'. Using fallback.")
                return self._generate_fallback_description(query, formatted_features)

            logger.info(f"Generated description for query '{query[:70]}...', length: {len(cleaned_text)} chars")
            return cleaned_text

        except Exception as e:
            logger.error(f"Error generating personalized text for query '{query[:70]}...': {e}", exc_info=True)
            return self._generate_fallback_description(query, "")

    def _clean_and_validate_description(self, text: str) -> str:
        """清理和验证描述文本"""
        # 移除常见的前缀
        text = re.sub(r"^(Okay, |Sure, |Certainly, |Here('s| is) (the|your) (generated|core) (paragraph|statement|representation|output):?)\s*\n*", "", text, flags=re.IGNORECASE).strip()
        text = re.sub(r"^(Generated Core Statement|Core Statement|Enhanced Query Representation|Research Profile|Profile|Description|Paragraph|Output)[:\-\s]*", "", text, flags=re.IGNORECASE).strip()
        text = re.sub(r"^(This research profile|The profile|The descriptive paragraph|The core statement)[:\-\s]*", "", text, flags=re.IGNORECASE).strip()
        ...
        if len(text) > self.max_length:
            logger.warning(f"Text exceeded max_length ({self.max_length}). Original length: {len(text)}. Truncating.")
        ...
        if not re.search(r'\b\w{2,}\b', text):
             logger.debug(f"Cleaned text '{text}' lacks substantial content after processing.")
             return ""
        return text.strip()
    ...
    def _generate_fallback_description(self, query: str, features: str) -> str:
        ...
        prefixes_to_remove = [
            "are there any research papers on ", "are there any studies that explore ",
            "are there any resources available for ", "are there any tools or studies that have focused on ",
            "are there papers that propose ", "are there studies that combine ", "what are ", "how to ", "can you find "
        ]
    ...
    def generate_personalized_text_batch(self, queries_data: List[Dict]) -> List[str]:
        ...
        for i, data_item in enumerate(queries_data):
            ...
            desc = self.generate_personalized_text(
                query_text,
                mem_res
            )
            if not desc:
                logger.error(f"CRITICAL: desc is empty even after primary fallback for query '{query_text[:70]}...'. Using generic.")
        ...
        logger.info(f"Batch generation complete. Generated {len(descriptions)} descriptions.")
        return descriptions
